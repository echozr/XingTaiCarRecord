<template>
	<view class="cmd-progress cmd-progress-default" :class="setStatusClass">
				...
				<view class="cmd-progress-inner" :style="setCircleStyle">
					...
					<block v-if="showInfo && !custom">
						<text class="cmd-progress-text" :title="setFormat">
							...
							<!-- #ifdef H5 -->
							<svg v-if="status == 'exception'" viewBox="64 64 896 896" data-icon="close" width="1em" height="1em" fill="currentColor" aria-hidden="true">
								<path
					...
					<block v-else>
						<view class="cmd-progress-custom"><slot></slot></view>
					</block>
					<!-- 状态文本 end -->
				</view>
			</view>
		...
		<block v-if="type == 'line'">
			...
			<block v-if="showInfo && !custom">
				<text class="cmd-progress-text" :title="setFormat">
					<block v-if="status != 'success' && status != 'exception' && setProgress < 100">{{ setFormat }}</block>
					<!-- #ifdef H5 -->
					<svg v-if="status == 'exception'" viewBox="64 64 896 896" data-icon="close-circle" width="1em" height="1em" fill="currentColor" aria-hidden="true">
						<path
		...
		<view class="text-center mt-1">{{text}}</view>
	</view>
</template>
